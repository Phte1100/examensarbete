...
const fetchArticlesAndThreads = async () => {
...
<template>
  <div class="max-w-4xl mx-auto p-6 space-y-8">
    ...
    <section>
      ...
      <!-- Lyssna på "updated"-eventet -->
      <Account @updated="fetchProfile" />
    </section>
    ...
    <section v-if="articles.length">
      ...
      <ul class="divide-y">
        ...
        >
          <div>
            <p class="text-sm text-black-500">{{ a.published_at?.substring(0, 10) }}</p>
            <a :href="a.url" class="text-base font-medium text-black hover:underline" target="_blank">
              {{ a.title }}
            </a>
            <p class="text-sm text-black-500">{{ a.source || 'Okänd' }}</p>
          </div>
          <button
    ...
    <section v-if="threads.length">
      ...
      <ul class="divide-y">
        ...
        >
          <div>
            <p class="text-sm text-black-500">{{ t.created_at?.substring(0, 10) }}</p>
            <a :href="t.url" class="text-base font-medium text-black hover:underline" target="_blank">
              {{ t.title }}
            </a>
            <p class="text-sm text-black-500">{{ t.source }}</p>
          </div>
          <button
