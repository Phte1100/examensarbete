<template>
  <div class="p-6">
    <form @submit.prevent="search" class="space-y-4">
      <div>
        <label class="block text-gray-700 font-semibold mb-1">Sök StackOverflow:</label>
        <input
          v-model="query"
          type="text"
          class="w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
          placeholder="t.ex. nuxt fetch auth"
        />
      </div>
    ...
    <div v-if="results.length" class="mt-6 space-y-4">
      <div v-for="(item, index) in results" :key="index" class="border p-4 rounded bg-gray-50">
        <a :href="item.link" target="_blank" class="text-lg font-semibold text-blue-700 hover:underline">
          {{ item.title }}
        </a>
    ...
    <p v-if="!loading && results.length === 0 && searched" class="mt-4 text-red-600 font-medium">
